...
2. **各工程完了時にメトリクスを計測し品質ゲートを通過すること**（`agents/review-and-quality.md` §21）
3. **方式設計ではFMEAを実施する**（RPN≥100はFTAも実施, `agents/formal-design.md` §24）
4. **詳細設計では前提/事後条件を記述する**（Design by Contract, `agents/formal-design.md` §23）
5. **レビューではプロトコルに従いウォークスルー/インスペクションを選択する**（`agents/review-and-quality.md` §26）
6. **プロジェクト完了時にレトロスペクティブを実施する**（`agents/multi-project-learning.md` §27）
7. **ファイル変更時は `dependency-map.md` で影響先を特定し、該当セクションだけチェックする**（`agents/consistency-and-change.md` §28）
...
### サブエージェント委譲（`agents/sub-agent-and-context.md` §9）

- 工程を跨ぐ際は必ず `docs/projects/<project>/context/` のコンテキストファイルを更新する
- サブエージェントに委譲する際は `agents/sub-agent-and-context.md` §9 の委譲指示テンプレートを使用する

### コンテキスト検索モード（標準優先）

- 運用モードは `standard` と `auto` の2つを基本とする（`vexp` 単独モードは設けない）
- まず拡張機能に依存しない `standard` 手順で作業可能な状態を確保する
- Vexpは任意の高速化レイヤとして扱い、利用不可時は標準手段で継続する
- 推奨運用は `auto`（利用可能ならVexp、不可なら標準へフォールバック）
...
1. 必ず `agents/process-and-traceability.md` §3 の**トレーサビリティ規約**に従ってタグを付与する
2. 成果物作成後、`knowledge/` 配下の観点表・チェックリストでセルフチェックを行う
3. 前工程の成果物がある場合は必ず読み込み、整合性を確認する
4. **設計書にはMermaid図を必ず含める**（`agents/mermaid-uml.md` §10参照）
5. **詳細設計ではソフトウェアの意図を日本語で詳しく書く**

### TDDワークフロー（`agents/coding-standards.md` §11）
...
- **関数は100行以内、サイクロマティック複雑度50以内**（`agents/coding-standards.md` §12）
- **セキュリティコーディング規約に従う**（`agents/coding-standards.md` §13）
- **テストダブル（Mock/Stub/Driver）を適切に使用する**（`agents/coding-standards.md` §14）
- **OSS選定時は致命的な脆弱性がないか確認する**（`agents/oss-and-compliance.md` §15, EU CRA準拠）

### 対話ログの記録（`agents/sub-agent-and-context.md` §19）
...
### OSSスニペット検出（`agents/oss-and-compliance.md` §18）
