...
## 15. AI記述コードのドキュメント規約 (Explainable by Design)

AIが生成するコードは「動く」だけでなく、「人間が読んで意図を即座に理解できる」ものでなければならない。

### 15.0 言語 (必須)

**コメントおよびDocstringは、原則として「日本語」で記述すること。**
（変数名や関数名は英語でよいが、その説明は日本語で行う）

### 15.1 ファイルヘッダー (必須)

すべてのソースファイルの先頭に、そのファイルの「責務」と「解決する問題」を記述すること。

```python
"""
module: user_service.py
description: ユーザー登録・認証およびプロフィール管理を行うドメインサービス。
note: 認証ロジックは AuthProvider に委譲し、ここではドメインルール（重複チェック等）に集中する。
"""
```

### 15.2 関数・クラス Docstring (必須)

すべての `public` なクラス・関数には Docstring を付与する。
引数・戻り値の説明に加え、**「なぜこの処理が必要か」「設計上の意図」** を含めること。
...
def calculate_discount(price: int, user_rank: str) -> int:
    ...
    intent: シルバー以上は一律10%だが、ゴールドは端数切り捨てルールが異なるため個別に分岐している。
    
    Args:
        price: 定価
        user_rank: 会員ランク ('gold', 'silver', 'bronze')
    Returns:
        割引後の価格
    """
```
...
アルゴリズムや、一見して直感的でない実装には必ずコメントで「理由」を添えること。

```python
# HACK: APIのレートリミットを回避するため、意図的に1秒スリープを入れている。
# 本来的にはキューイングすべきだが、現状のトラフィック量ならこれで十分と判断。
time.sleep(1)
```
