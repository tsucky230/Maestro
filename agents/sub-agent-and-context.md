...
`conversation-log.md` は構造化サマリーだが、必要に応じて「チャット欄の入出力に近いログ」を追加保存できる。

1. **デフォルトはOFF**
2. 人間が「**ログを取って**」「**生ログを開始して**」等を指示したときのみ開始
3. 停止指示（例: 「ログ停止」「生ログ終了」）で記録を終了
4. 保存先:
   - フレームワーク更新（main相当）: `LOG/`
   - プロジェクト作業: `docs/projects/<project>/logs/raw-chat/`
5. 形式: `docs/templates/chat-transcript-log.md` を使用
6. Git管理方針:
   - フレームワーク更新（main相当）は `LOG/` を `.gitignore` 対象にする
   - プロジェクト作業は `.gitignore` に追加するかを人間に確認して選択する
7. 生ログ保存時の必須記録:
   - モデル名
   - 収集開始時刻 / 収集終了時刻
   - 消費トークン（入力 / 出力 / 合計）
