...
2. **各工程完了時にメトリクスを計測し品質ゲートを通過する**（Section 21）
3. **方式設計ではFMEAを実施する**（Section 24）
4. **レビューではウォークスルー/インスペクション技法を選択する**（Section 26）
5. **ファイル変更時は `dependency-map.md` で影響先を特定し整合性チェックする**（Section 28）
...
- **対話ログを記録する**（`docs/templates/conversation-log.md` テンプレート使用, AGENTS.md Section 19）
