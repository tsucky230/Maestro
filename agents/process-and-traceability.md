# プロセス基盤・トレーサビリティ・USDM

> [!IMPORTANT]
> **最優先ルール: ブランチ戦略の厳守**
>
> - すべての新規開発、修正、ドキュメント作成は、必ず**新しいブランチ**を作成して行うこと。
> - `main` ブランチへの直接コミットは**厳禁**とする。
> - ブランチ命名規則: `ftr_` (機能), `fix/` (修正), `docs/` (文書), `refactor/` (リファクタ) 等のプレフィックスを使用すること。

> 📈 **このファイルの役割**: V字モデルに基づく開発プロセス（SWP.1〜SWP.6）、USDM（要求仕様書）記法、および成果物間のトレーサビリティ（追跡可能性）を確保するルールを定義します。

> このファイルは `AGENTS.md` Section 2, 3, 4, 6, 8 の内容を含みます。

---

## 2. ESPR 2.0 プロセスフロー（V字モデル）

```
要求仕様定義 (SWP.1) ←――――――――――――→ 適格性確認テスト (SWP.6)
    ↓                                        ↑
方式設計 (SWP.2) ←――――――――――――→ 結合テスト (SWP.5)
    ↓                                    ↑
詳細設計 (SWP.3) ←――――――――→ 単体テスト (SWP.4)
    ↓                            ↑
    └──→ 製造（コーディング） ──→┘
```

### 各工程の定義

| 工程ID | 工程名 | 入力成果物 | 出力成果物 | テンプレート |
|--------|--------|------------|------------|-------------|
| SWP.1 | 要求仕様定義 | 顧客要求、制約事項 | 要求仕様書 | `docs/templates/requirements-spec.md` |
| SWP.2 | 方式設計 | 要求仕様書 | 方式設計書 | `docs/templates/architecture-design.md` |
| SWP.3 | 詳細設計 | 方式設計書 | 詳細設計書 | `docs/templates/detailed-design.md` |
| SWP.4 | 単体テスト | 詳細設計書、ソースコード | 単体テスト仕様書・結果 | `docs/templates/test-design.md` |
| SWP.5 | 結合テスト | 方式設計書 | 結合テスト仕様書・結果 | `docs/templates/test-design.md` |
| SWP.6 | 適格性確認テスト | 要求仕様書 | 適格性確認テスト仕様書・結果 | `docs/templates/test-design.md` |

---

## 3. トレーサビリティ規約

### タグ体系

すべての成果物にトレーサビリティタグを付与する。タグの書式:

```
[プレフィックス]-[連番3桁]  例: REQ-001
```

| プレフィックス | 対象 | 例 |
|------------|------|-----|
| `REQ` | 要求 | `REQ-001` |
| `SPEC` | 仕様（要求に紐づく） | `SPEC-001` |
| `ARC` | 方式設計要素 | `ARC-001` |
| `DET` | 詳細設計要素 | `DET-001` |
| `SRC` | ソースコード実装 | `SRC-001` |
| `UT` | 単体テストケース | `UT-001` |
| `IT` | 結合テストケース | `IT-001` |
| `QT` | 適格性確認テストケース | `QT-001` |

### トレース方向

```
REQ-001
  └─ SPEC-001, SPEC-002      (要求→仕様)
       └─ ARC-001             (仕様→方式設計)
            └─ DET-001        (方式設計→詳細設計)
                 ├─ SRC-001   (詳細設計→ソースコード)
                 └─ UT-001    (詳細設計→単体テスト)
       └─ IT-001              (仕様→結合テスト)
  └─ QT-001                   (要求→適格性確認テスト)
```

### タグ付与ルール

1. **新規成果物作成時**: 必ずトレーサビリティタグを付与する
2. **上位参照**: 各成果物に `traces-to:` フィールドで上位成果物タグを記載する
3. **ソースコード**: 関数・クラスのDocstring/コメントに `@trace DET-XXX` を記載する
4. **テストコード**: テスト関数名またはDocstringに `@trace UT-XXX` を記載する

---

## 4. USDM (Universal Specification Describing Manner) 規約

要求仕様はUSDM形式で記述する。各要求は以下の4要素を含む:

| 要素 | 説明 | 記述ルール |
|------|------|-----------|
| **要求** | 実現してほしいこと | 目的語＋動詞で簡潔に記述 |
| **理由** | なぜその要求が必要か | 背景・根拠を明記 |
| **説明** | 補足事項 | 用語定義、具体例、制約条件 |
| **仕様** | あるべき姿 | 検証可能な具体的記述 |

### 記述例

```markdown
### REQ-001: ユーザー認証機能

**要求**: システムはユーザーを認証できること

**理由**: 不正アクセスを防止し、ユーザーごとのデータを安全に管理するために、
本人確認の仕組みが必要である。

**説明**: 認証方式はメールアドレス＋パスワードを基本とし、将来的にOAuth連携も
検討する。パスワードはbcryptでハッシュ化して保存する。

#### 仕様

- **SPEC-001**: メールアドレスとパスワードによるログイン機能を提供する
  - traces-to: REQ-001
- **SPEC-002**: パスワードはbcrypt（コスト因子12以上）でハッシュ化して保存する
  - traces-to: REQ-001
- **SPEC-003**: ログイン失敗は5回連続でアカウントを30分間ロックする
  - traces-to: REQ-001
```

---

## 5. 要求抽出の完全性プロトコル (厳守)

> **最優先事項**: 機能要件・非機能要件の抽出において、AIエージェントは情報の**抜粋や要約を絶対に行わない**こと。原本に記載されている全ての要件を、一つ残らず網羅的にリストアップする。「完全性」が最優先である。

### 1. 網羅性の原則 (Completeness First)

- 原本ドキュメント（議事録、顧客資料、旧仕様書）に記載されている要件は、**全て**リストアップする。
- 「重要そうなものだけ」「概要のみ」といった判断による選別は**禁止**とする。
- 明らかに不要と思われる情報であっても、まずはリストアップし、その後の工程で「対象外」と判定するプロセスを経ること。

### 2. 原文保持と非要約 (No Summarization)

- 要件の記述は、原則として**原文の表現を維持**する。
- 勝手な要約、意訳、箇条書きへの短縮を行わない。
- USDM化に伴う整形は許可されるが、情報の欠落（例えば詳細な数値や条件の省略）は許容されない。

---

## 6. AI推論・分析プロトコル (推奨)

AIエージェントは、抽出した要件に対して以下の**推論・分析**を行い、「説明」または「備考」欄に追記すること。
ただし、推論部分は必ず「(推論)」や「(推定)」と明記し、事実と区別すること。

### 1. ビジネスインパクトの推論

- 要求の背景や文脈から、**営業上の重要ポイント**や**コスト影響**を推論する。
- 記載例:
  - `[推論] ビジネスインパクト: 競合A社との差別化要因となるため、営業上の重要度が高い。`
  - `[推論] コスト影響: 専用ハードウェアの追加が必要になる可能性があり、コスト増のリスク大。`

### 2. 工数・優先度の推論

- 明記されていない場合でも、過去の知見や一般的複雑度から**工数見積もり**や**開発優先度**を推論する。
- 記載例:
  - `[推論] 開発優先度: 高 (認証機能は他機能の前提となるため)`
  - `[推論] 工数見積: 3人日 (バリデーションロジックが複雑なため)`

---

## 7. 例外処理・柔軟性プロトコル (セーフティネット)

カテゴリ分類の固定化による抽出漏れを防ぐため、AIエージェントは以下の例外処理を行う権限を持つ。

### 1. 例外的なサブセクションの自律生成

- 標準カテゴリに分類できず、かつプロジェクト成功に**不可欠**だと判断される重要要件がある場合、AIは自律的に新たなサブセクションを作成してよい。
  - 例: `#### 医療業界固有のレギュレーション`
  - 条件: 新設したサブセクションには、必ず「なぜそのカテゴリが必要か」の**理由を付記**すること。

### 2. その他要件の集約

- 分類不能だが重要度が低い要件については、無理に独立させず `#### その他の要件` にまとめること。

---

## 8. 作業フロー

### 要求ヒアリング時

1. ゴール（達成したいこと）を聞き取る
2. リスク・制約条件を確認する
3. **組み込みプロジェクトの場合**: `knowledge/embedded-constraints.md` §1「逆質問テンプレート」を使い、HW境界条件を漏れなく収集する
4. 機能要求と非機能要求を分離する
5. **組み込みプロジェクトの場合**: `knowledge/embedded-constraints.md` §2「非機能要求テンプレート」に基づき、リソース制約・タイミング制約・安全性要求を仕様化する
6. USDMフォーマットで要求仕様書を作成する
7. トレーサビリティタグを付与する

### 設計・製造時

1. 前工程の成果物を読み込む
2. 知見ライブラリを参照する
3. トレーサビリティタグを引き継いで新規成果物を作成する
4. 作成した成果物をレビュー観点表でセルフチェックする
5. トレーサビリティマトリクスを更新する

### レビュー依頼時

1. 依頼された成果物を読み込む
2. 前工程・次工程の成果物との整合性を確認する
3. レビュー観点表に基づいて指摘を行う
4. トラブル事例との類似性を確認する
5. 指摘一覧をレビュー指摘タグ付きで出力する

---

## 8. 命名規則・用語集

| 用語 | 定義 |
|------|------|
| 要求 | ステークホルダーがシステムに求める事柄 |
| 仕様 | 要求を満たすためのシステムのあるべき姿 |
| トレーサビリティ | 成果物間の追跡可能性 |
| 方式設計 | システム全体の構造・方針を決定する設計 |
| 詳細設計 | モジュール・関数レベルの具体的設計 |
| 適格性確認 | 要求仕様を満たしているかの最終検証 |
| USDM | Universal Specification Describing Manner |
| V字モデル | 開発工程とテスト工程を対応させた開発モデル |
| TDD | テスト駆動開発（Test-Driven Development） |
| SBOM | ソフトウェア部品表（Software Bill of Materials） |
| CC | サイクロマティック複雑度（Cyclomatic Complexity） |
